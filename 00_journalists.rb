...
puts  "il y a #{handle.count} handle" #compte le nombre d'élément dans le tableaux

#Quel est le handle le plus court de cette liste ?

handle_shorter = handle.sort_by(&:length) #tri par ordre de longeur

puts "cette handle est le plus court: #{handle_shorter.first}" #donne le premier éléments de la liste trié

#Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)

handle_cinq = handle.select {|elem|elem.length==6}.count

puts "Il y a #{handle_cinq}handles avec 5 caractères"

#Combien commencent par une majuscule (première lettre juste après le @) ?

handle_maj = handle.select {|elem|elem =~ /^@[A-Z]/}.count

puts "Il y a #{handle_maj} handles avec des majuscules !"

#Trie la liste de handle par ordre alphabétique.

puts handle_tri_abc = handle.sort_by {|elem|elem.downcase}

#Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)

puts handle_S_to_B = handle.sort_by {|elem|elem.length}

#Quelle est la position dans l'array de la personne @epenser ?

puts "@epenser est en #{handle.index('@epenser')} ème position !"

#Sors-moi une répartition des handle par taille de ces derniers !

handlesgrp = handle.sort_by(&:length).group_by(&:length)
handlesgrp.each do |length, handle|
    puts "#{length} caractères"
    puts "#{handle.count}"
end